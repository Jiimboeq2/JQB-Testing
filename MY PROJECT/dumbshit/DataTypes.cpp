...
bool RIType::ToText(LSOBJECTDATA ObjectData, char *buf, unsigned int buflen)
{
	...
	strcpy_s(buf,buflen,"ISXRI");
	return true;
}

bool RIType::GetMember(LSOBJECTDATA ObjectData, PLSTYPEMEMBER pMember, int argc, char *argv[], LSOBJECT &Object)
{
	...
	switch((RITypeMembers)pMember->ID)
	{
	case RetrieveData:
...
bool RIType::GetMethod(LSOBJECTDATA &ObjectData, PLSTYPEMETHOD pMethod, int argc, char *argv[])
{
	...
	switch((RITypeMethods)pMethod->ID)
	{
	case PerformAction:
